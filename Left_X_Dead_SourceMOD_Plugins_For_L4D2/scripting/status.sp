...
public OnPluginStart()
{
    ...
    cvar_HumansNeeded = CreateConVar("sm_l4dstats_minhumans", "2", "达到多少玩家后才进行统计", FCVAR_PLUGIN, true, 1.0, true, 4.0);
    cvar_UpdateRate = CreateConVar("sm_l4dstats_updaterate", "90", "间隔多少秒更新数据并通告", FCVAR_PLUGIN, true, 30.0);
    cvar_AnnounceMode = CreateConVar("sm_l4dstats_announcemode", "0", "赚取积分后是否在游戏内通告. 0 = 不显示, 1 = 仅显示给玩家, 2 = 爆头才显示给玩家, 3 = 所有人都可以看见", FCVAR_PLUGIN, true, 0.0, true, 3.0);
    cvar_MedkitMode = CreateConVar("sm_l4dstats_medkitmode", "0", "奖励模式. 0 = 根据生命和爆头数, 1 = 根据生命", FCVAR_PLUGIN, true, 0.0, true, 1.0);
    cvar_SiteURL = CreateConVar("sm_l4dstats_siteurl", "http://www.sy64.com/l4d/", "排名网址", FCVAR_PLUGIN);
    cvar_NewBiesSiteURL = CreateConVar("sm_l4dstats_newbiessiteurl", "http://l4d.sy64.com/newbies.htm", "未注册玩家自动打开的网页地址", FCVAR_PLUGIN);
    cvar_StatusName = CreateConVar("sm_l4dstats_statusname", "[L4D] 58.61.167.197:27016", "未注册玩家自动打开的网页地址", FCVAR_PLUGIN);
    cvar_RankOnJoin = CreateConVar("sm_l4dstats_rankonjoin", "0", "当玩家连接的时候显示统计信息推荐禁用,可避免多余数据库查询. 0 = 禁用, 1 = 启用", FCVAR_PLUGIN, true, 0.0, true, 1.0);
    cvar_SilenceChat = CreateConVar("sm_l4dstats_silencechat", "0", "输入rank和top10是否. 0 = 显示, 1 = 不显示", FCVAR_PLUGIN, true, 0.0, true, 1.0);
    cvar_DisabledMessages = CreateConVar("sm_l4dstats_disabledmessages", "1", "显示 '统计禁用' 消息, 允许聊天指令当统计被禁用后. 0 = 屏蔽消息/禁止聊天, 1 = 显示消息/允许聊天", FCVAR_PLUGIN, true, 0.0, true, 1.0);


    // Game mode Cvars
    cvar_EnableCoop = CreateConVar("sm_l4dstats_enablecoop", "1", "启用/禁用 COOP 统计功能", FCVAR_PLUGIN, true, 0.0, true, 1.0);
    
    stat_cheat = CreateConVar("sm_stat_cheats", "1", "启用/禁用 和LXD通讯作弊功能", FCVAR_PLUGIN, true, 0.0, true, 4.0);
    buyTimeout = CreateConVar("rank_buy_timeout","10", "玩家二次购买所需要等待的时间",FCVAR_PLUGIN,true,0.0);

    // Infected point Cvars
    cvar_Infected = CreateConVar("sm_l4dstats_infected", "1", "杀死感染者获得积分", FCVAR_PLUGIN, true, 1.0);
    cvar_Hunter = CreateConVar("sm_l4dstats_hunter", "2", "杀死Hunter获得积分", FCVAR_PLUGIN, true, 1.0);
    cvar_Smoker = CreateConVar("sm_l4dstats_smoker", "3", "杀死Smoker获得积分", FCVAR_PLUGIN, true, 1.0);
    cvar_Boomer = CreateConVar("sm_l4dstats_boomer", "5", "杀死Boomer获得积分", FCVAR_PLUGIN, true, 1.0);

    // Misc personal gain Cvars
    cvar_Pills = CreateConVar("sm_l4dstats_pills", "1", "给予队友药丸获得积分", FCVAR_PLUGIN, true, 1.0);
    cvar_Medkit = CreateConVar("sm_l4dstats_medkit", "20", "给予队友医疗包获得积分", FCVAR_PLUGIN, true, 1.0);
    cvar_SmokerDrag = CreateConVar("sm_l4dstats_smokerdrag", "5", "拯救队友从Smoker抓下获得积分", FCVAR_PLUGIN, true, 1.0);
    cvar_ChokePounce = CreateConVar("sm_l4dstats_chokepounce", "10", "从Hunter的突袭和Smoker的烟雾内拯救队友获得的积分", FCVAR_PLUGIN, true, 1.0);
    cvar_Revive = CreateConVar("sm_l4dstats_revive", "15", "拯救无行为能力的队友获得的积分", FCVAR_PLUGIN, true, 1.0);
    cvar_Rescue = CreateConVar("sm_l4dstats_rescue", "10", "营救队友获得的积分", FCVAR_PLUGIN, true, 1.0);
    cvar_Protect = CreateConVar("sm_l4dstats_protect", "3", "保护队友获得的积分", FCVAR_PLUGIN, true, 1.0);

    // Team gain Cvars
    cvar_Tank = CreateConVar("sm_l4dstats_tank", "25", "团队杀死TANK获得的积分", FCVAR_PLUGIN, true, 1.0);
    cvar_Panic = CreateConVar("sm_l4dstats_panic", "25", "尸潮无人倒下获得积分", FCVAR_PLUGIN, true, 1.0);
    cvar_BoomerMob = CreateConVar("sm_l4dstats_boomermob", "10", "被Boomer溅射后无人倒下获得积分", FCVAR_PLUGIN, true, 1.0);
    cvar_SafeHouse = CreateConVar("sm_l4dstats_safehouse", "10", "到达安全屋获得的积分", FCVAR_PLUGIN, true, 1.0);
    cvar_Witch = CreateConVar("sm_l4dstats_witch", "10", "打扰女巫减少的积分", FCVAR_PLUGIN, true, 1.0);
    cvar_Campaign = CreateConVar("sm_l4dstats_campaign", "5", "完成战役获得的积分", FCVAR_PLUGIN, true, 1.0);

    // Point loss Cvars
    cvar_FFire = CreateConVar("sm_l4dstats_ffire", "25", "伤害队友减少多少积分", FCVAR_PLUGIN, true, 1.0);
    cvar_FIncap = CreateConVar("sm_l4dstats_fincap", "75", "纵火烧伤队友扣除的积分", FCVAR_PLUGIN, true, 1.0);
    cvar_FKill = CreateConVar("sm_l4dstats_fkill", "250", "杀死队友所扣除的积分", FCVAR_PLUGIN, true, 1.0);
    cvar_InSafeRoom = CreateConVar("sm_l4dstats_insaferoom", "5", "让感染者进入安全屋扣除的积分", FCVAR_PLUGIN, true, 1.0);
    cvar_Restart = CreateConVar("sm_l4dstats_restart", "100", "关卡重启所扣除的积分", FCVAR_PLUGIN, true, 1.0);
    
    cvar_buyshotgun = CreateConVar("sm_buy_shotgun", "50", "购买散弹枪需要的积分", FCVAR_PLUGIN, true, 1.0);
    cvar_buysmg = CreateConVar("sm_buy_smg", "50", "购买冲锋枪需要的积分", FCVAR_PLUGIN, true, 1.0);
    cvar_buyrifle = CreateConVar("sm_buy_rifle", "1000", "购买步枪需要的积分", FCVAR_PLUGIN, true, 1.0);
    cvar_buyhrifle = CreateConVar("sm_buy_hrifle", "1000", "购买狙击步枪需要的积分", FCVAR_PLUGIN, true, 1.0);
    cvar_buyautoshotgun = CreateConVar("sm_buy_autoshotgun", "1000", "购买自动散弹枪需要的积分", FCVAR_PLUGIN, true, 1.0);
    cvar_buypistol = CreateConVar("sm_buy_pistol", "100", "购买手枪需要的积分", FCVAR_PLUGIN, true, 1.0);
    cvar_buypipebomb = CreateConVar("sm_buy_pipebomb", "200", "购买土制炸弹需要的积分", FCVAR_PLUGIN, true, 1.0);
    cvar_buymolotov = CreateConVar("sm_buy_molotov", "150", "购买燃烧弹需要的积分", FCVAR_PLUGIN, true, 1.0);
    cvar_buypills = CreateConVar("sm_buy_pills", "100", "购买药丸需要的积分", FCVAR_PLUGIN, true, 1.0);
    cvar_buykit = CreateConVar("sm_buy_kit", "800", "购买医疗包需要的积分", FCVAR_PLUGIN, true, 1.0);
    cvar_buyrp = CreateConVar("sm_buy_rp", "100", "购买彩票需要的积分", FCVAR_PLUGIN, true, 1.0);
    cvar_buyjn = CreateConVar("sm_buy_jn", "5000", "随机购买技能所需积分", FCVAR_PLUGIN, true, 1.0);
    cvar_buydb = CreateConVar("sm_buy_db", "10", "赌博赔率 10就是 1:10", FCVAR_PLUGIN, true,1.0,true,18.0);
    cvar_buylife = CreateConVar("sm_buy_life", "1500", "购买快捷补血所需花费积分", FCVAR_PLUGIN, true, 1.0);
    cvar_buyjd = CreateConVar("sm_buy_jd", "500", "购买解毒剂所需要积分", FCVAR_PLUGIN, true, 1.0);
    
    cvar_vip_buyrifle = CreateConVar("sm_buy_vip_rifle", "30000", "购买VIPM16需花费的积分", FCVAR_PLUGIN, true, 1.0);
    cvar_vip_buyautoshotgun = CreateConVar("sm_buy_vip_autoshotgun", "30000", "购买VIP自动散弹枪需花费的积分", FCVAR_PLUGIN, true, 1.0);
    cvar_vip_buyhrifle = CreateConVar("sm_buy_vip_hrifle", "30000", "购买VIP狙击步枪需花费的积分", FCVAR_PLUGIN, true, 1.0);
    
    cvar_vip_buykit = CreateConVar("sm_buy_vip_kit", "25000", "购买VIP药包需花费的积分", FCVAR_PLUGIN, true, 1.0);
    cvar_vip_buypills = CreateConVar("sm_buy_vip_pills", "888888", "购买VIP止痛药需花费的积分", FCVAR_PLUGIN, true, 1.0);
    cvar_vip_buybome = CreateConVar("sm_buy_vip_bome", "20000", "购买VIP土制炸弹需花费的积分", FCVAR_PLUGIN, true, 1.0);
    cvar_vip_buymolotov = CreateConVar("sm_buy_vip_molotov", "20000", "购买VIP土制炸弹需花费的积分", FCVAR_PLUGIN, true, 1.0);
    
    cvar_changteamprc = CreateConVar("sm_changeteam_prc", "10000", "转换到感染者团队所需积分", FCVAR_PLUGIN, true, 1.0);
    
    cvar_call_add = CreateConVar("sm_call_add", "100", "召唤一只僵尸所需要的积分", FCVAR_PLUGIN, true, 1.0);
    cvar_call_addevent = CreateConVar("sm_call_addevent", "5000", "召唤一次尸群事件需要的积分", FCVAR_PLUGIN, true, 1.0);
    cvar_call_hunter = CreateConVar("sm_call_hunter", "3000", "召唤hunter所需要的积分", FCVAR_PLUGIN, true, 1.0);
    cvar_call_boomer = CreateConVar("sm_call_boomer", "3000", "召唤boomer所需要的积分", FCVAR_PLUGIN, true, 1.0);
    cvar_call_smoker = CreateConVar("sm_call_smoker", "3000", "召唤smoker所需要的积分", FCVAR_PLUGIN, true, 1.0);
    cvar_call_witch = CreateConVar("sm_call_witch", "10000", "召唤witch所需要的积分", FCVAR_PLUGIN, true, 1.0);
    cvar_call_tank = CreateConVar("sm_call_tank", "30000", "召唤tank所需要的积分", FCVAR_PLUGIN, true, 1.0);
    
    cvar_usetime = CreateConVar("sm_useitem_time", "720", "通过VIP菜单购买武器后可使用的时间单位分钟 720 = 12 小时", FCVAR_PLUGIN, true, 1.0);
    cvar_tundertime = CreateConVar("sm_tunder_time", "2", "购买了卫星炮技能后能够使用的次数", FCVAR_PLUGIN, true, 1.0);
    cvar_point_tunder = CreateConVar("sm_buy_point_tunder", "10000", "购买卫星炮技能所需要的价格", FCVAR_PLUGIN, true, 1.0);
    cvar_tunderpower = CreateConVar("sm_tunder_power", "50000", "卫星炮每次攻击所造成的伤害", FCVAR_PLUGIN, true, 1.0);
    cvar_tunderwaittime = CreateConVar("sm_tunder_waittime", "30", "卫星炮技冷却延时", FCVAR_PLUGIN, true, 0.0);
    cvar_powershotwaittime = CreateConVar("sm_powershot_waittime", "5", "榴弹炮技冷却延时", FCVAR_PLUGIN, true, 0.0);
    
    cvar_god_point = CreateConVar("sm_god_point", "9999", "购买OB牌居家必备丸增加到的血量", FCVAR_PLUGIN, true, 1.0);
    cvar_god_money = CreateConVar("sm_god_money", "50000", "购买OB牌居家必备丸所需要的积分", FCVAR_PLUGIN, true, 1.0);
    
    cvar_powershot_money = CreateConVar("sm_powershot_money", "5000", "购买榴弹炮需要的积分", FCVAR_PLUGIN, true, 1.0);
    cvar_powershot_round = CreateConVar("sm_powershot_round", "10", "购买榴弹炮后可以使用的次数", FCVAR_PLUGIN, true, 1.0);
    
    cvar_numchange = CreateConVar("sm_number_changeteam", "3", "通过购买能跳到感染者团队的玩家数量", FCVAR_PLUGIN, true, 0.0);
...
public Action:buy(client, args) 
{
	...
	if(client <= 0)
	{
		ReplyToCommand(client, "\x01[SM] 请在游戏中输入方可!\x03");
		return;
	}
	...
	{
		ReplyToCommand(client, "\x01[SM] 并未开启购买系统!\x03");
		return;
	}
...
public ConnectDB()
{
        ...
        KvSetString(kv, "database", "l4d");
        KvSetString(kv, "user", "l4d");
...
CheckPlayerDB(client)
{
    ...
    // if (strlen(SteamID) == 0 || strlen(SteamID) <= 2 || strlen(SteamID) >= 30) return;
...
public Action:UpdatePlayer(Handle:timer,any:client)
{
    ...
    decl String:ServerName[255],pieces[4];
...
public DisplayRank(Handle:owner, Handle:hndl, const String:error[], any:data)
{
    ...
    {
        Format(Value, sizeof(Value), "完整的统计信息请访问 %s", URL);
        DrawPanelText(RankPanel, Value);
    }
